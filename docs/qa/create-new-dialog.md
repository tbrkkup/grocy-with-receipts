...
- **Abgedeckt:** Öffnen des gestapelten Dialogs, Anlegen, Rückmeldung ins Ziel-`<select>`,
  Auswahl der neuen Option, kein Reload (Formularzustand erhalten), Dialog schließt.
  Das getestete Produktgruppen-Feld ist ein **einfaches `<select>`** (kein combobox).
- **Noch offen (Phase C/D):** combobox-umhüllte Selects (`combobox.refresh()`-Pfad),
  Standort/Einheiten/Geschäft im Produktformular, weitere Formulare (Einkauf, Verbrauch,
  Inventur, Umlagerung, Einkaufsliste), Dialog-über-Dialog-über-Dialog (Picker liegt selbst
