...
2. **Upload:** PDF **oder Foto/Scan** (JPG/PNG) per Drag & Drop oder Dateidialog (v18)
3. **Analyse:**
   - Digital-PDF: pdf.js-Text → Call 1 extrahiert Produktdaten + Datum + Geschäft
   - Foto/Scan (v18): Bild direkt an Claude **Vision** → gleiches JSON wie Call 1 (kein OCR)
   - Call 2: Produkte semantisch mit Grocy-Stammdaten abgleichen (beide Pfade)
...
| v20 | Neues Produkt aus Link: Im Neuanlegen-Dialog kann ein Produkt-Link abgerufen werden – Claude zieht Name, Gewicht (→ Beschreibung) und Produktbild (→ Grocy-Produktbild) von der Seite. Abruf über neuen serverseitigen Proxy `public/url-proxy.php` (umgeht CORS, SSRF-Schutz + API-Key-Pflicht). **Muss wie das Widget nach `/public/` deployt werden.** Bild-/Seitenabruf ist nicht fatal – schlägt er fehl, wird das Produkt trotzdem angelegt |
...
- v21 Rabatt-Einrechnung headless getestet (`scratchpad/test-discount.js`: Aldi-Süd-Beleg, Nettopreise + Summe 22,61, Rabatt-über/unter-Produkt, keine Rabatt-Posten). **Vom Nutzer zum Testen vorgemerkt (2026-07-06), noch nicht live bestätigt.**
- v22 dm-MwSt-Indikator + Geschenkkarten-Schutz headless getestet (`scratchpad/test-discount.js` giftOk). Prompt-Anpassung (dm-Ziffer als MwSt-Satz statt Menge) nur per Prompt – End-to-End gegen echten dm-Beleg durch den Nutzer offen (Beispielbeleg vorhanden).
